...
import { Activity, TrendingUp, Eye, BarChart2, Camera, MousePointer2, ArrowRightLeft, ArrowUpCircle, User, Target } from 'lucide-react';
                    ...
                    <div className="mt-2">
                      <div className="flex justify-between items-end mb-3">
                        <span className="text-xs text-gray-500 font-bold flex items-center gap-1">
                          <Eye size={14} className="text-blue-500" /> 両眼視差
                        </span>
                        <span className="text-3xl font-bold text-gray-800">
                          {currentDetail.c1_disparity.toFixed(4)}°
                        </span>
                      </div>
                      <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-3 rounded-lg border border-blue-200 h-12 flex items-center">
                        <div className="flex items-center justify-between w-full">
                          <span className="text-xs text-blue-600 font-medium">奥行き知覚精度</span>
                          <div className="flex items-center gap-2">
                            <div className="w-24 h-2 bg-blue-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-blue-500 rounded-full transition-all duration-300"
                                style={{ width: `${Math.min(100, currentDetail.c1_disparity * 100)}%` }}
                              ></div>
                            </div>
                            <span className="text-xs font-bold text-blue-700">基準値</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-orange-500">
                  ...
                  {graphMode === 'trajectory' ? (
                    ...
                    <div className="mt-2">
                      <div className="flex justify-between items-end mb-3">
                        <span className="text-xs text-gray-500 font-bold flex items-center gap-1">
                          <Eye size={14} className="text-orange-500" /> 両眼視差
                        </span>
                        <span className="text-3xl font-bold text-gray-800">
                          {currentDetail.c2_disparity.toFixed(4)}°
                        </span>
                      </div>
                      <div className={`p-3 rounded-lg border h-12 flex items-center ${currentDetail.c2_disparity >= currentDetail.c1_disparity ? 'bg-gradient-to-r from-green-50 to-green-100 border-green-200' : 'bg-gradient-to-r from-red-50 to-red-100 border-red-200'}`}>
                        <div className="flex items-center justify-between w-full">
                          <span className={`text-xs font-medium ${currentDetail.c2_disparity >= currentDetail.c1_disparity ? 'text-green-600' : 'text-red-600'}`}>基準との比較</span>
                          <div className="flex items-center gap-2">
                            {currentDetail.c2_disparity > currentDetail.c1_disparity ? (
                              <div className="flex items-center text-green-600 font-bold">
                                <TrendingUp className="w-4 h-4 mr-1" />
                                <span className="text-lg">{((currentDetail.c2_disparity / currentDetail.c1_disparity - 1) * 100).toFixed(1)}%</span>
                                <span className="text-xs ml-1 font-medium">向上</span>
                              </div>
                            ) : (
                              <div className="flex items-center text-red-600 font-bold">
                                <TrendingUp className="w-4 h-4 mr-1" transform="scale(1, -1)" />
                                <span className="text-lg">{((1 - currentDetail.c2_disparity / currentDetail.c1_disparity) * 100).toFixed(1)}%</span>
                                <span className="text-xs ml-1 font-medium">低下</span>
                              </div>
                            )}
                          </div>
                        </div>
